...
public class Step2 {    // class s
    ...
    public static void main(String[] args) {    // main s
        ...
        longValue = 100;
        float floatValue = longValue;   // long(8바이트) --> float(4바이트) : 묵시적 형 변환 ( 부동소수점의 표현 단위 더 크기때문에, 더 정밀하기 때문에 )
        ...
        int result1 = b1 + b2;  // byte + byte = int , result1의 자료형은 int이다.
        ...
        double result4 = i1 + d1;   // int + double = double

        // 2. 명시적/강제 형 변환
            // 1. 메모리 크키가 큰 것부터 작은 순으로 변환
            // 2. 데이터 손실이 있을 수 있다.
            // (변환할자료형)변수명
            // byte <- short <- int <- long <- float <- double

        int intValue2 = 10;
        // byte byteValue2 = intValue2;    // 오류 : 4바이트를 1바이트에 저장할 수 없음.
        byte byteValue2 = (byte)intValue2;  // 강제 형 변환
