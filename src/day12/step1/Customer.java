...
public class Customer {
    ...
    // 1. 멤버변수  : 객체 내 속성값을 저장할 변수 , 접근제한자 static 타입 변수명
        // 접근제한자
        // - private : 다른 클래스로부터 접근 제한 , 현재 클래스에서만 호출이 가능하다.
        // - public : 다른 클래스로부터 접근 가능 , 현재 외 다른 클래스에서도 호츌이 가능하다.
        // 정적변수
    ...
    // 3. 메소드 : 함수 , 객체의 방법/행위/행동 정의
        // 접근제한자 static 반환타입 함수명(매개변수1 , 매개변수2){ 실행문; return 값};
    public int calcPrice(int price){
        this.bonusPoint += price * this.bonusRatio;
        // this란 ? 매개변수와 멤버변수 구분하기 위해 , 해당 함수를 실행한 객체를 식별용
        return price;   // return 값 , 값은 메소드 타입 타입과 일치한 값 , 반환 값이 없으면 void
    }
