       ...
       /* System.out.println(categoryList);
        ...
        while(true){
            ...
            if(ch == 1){
                ...
                /*for(int i = 0 ; i < categoryList.size() ; i++){
                    // 카테고리명이 한번만 나오게 해야함 // 변수를 정하지않을 경우 카테고리 1번 나오고 멈춤...?
                    int printCateCode = categoryList.get(i).getCategoryCode();
                    System.out.println(">>" + categoryList.get(i).getCategoryName() + " 카테고리 제품목록 -");
                    for(int j = 0 ; j < productList.size() ; j++){
                        if(productList.get(j).getCategoryCode() == printCateCode ){
                            System.out.println("제품코드 : " + productList.get(i).getProductCode() +
                                    "    제품명 : " + productList.get(i).getProductName() + "    제품가격 : " + productList.get(i).getProductPrice());
                        }
                    }
                }*/
                for(int i = 0 ; i < categoryList.size() ; i++){
                    ...
                    System.out.println(">>" + categoryList.get(i).getCategoryName() + " 카테고리 제품목록 ");
                    for(int j = 0 ; j < productList.size() ; j++){
                        if(productList.get(j).getCategoryCode() == categoryList.get(i).getCategoryCode()){
                            System.out.println("제품코드 : " + productList.get(j).getProductCode() +
                                    "    제품명 : " + productList.get(j).getProductName() + "    제품가격 : " + productList.get(j).getProductPrice());
                        }
                    }
                }
